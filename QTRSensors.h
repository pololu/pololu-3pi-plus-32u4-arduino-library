...
class QTRSensors
{
  ...
  protected:
    ...
    virtual void initEmitterPin(uint8_t pin)
    {
      pinMode(pin, OUTPUT);
    }

    virtual void releaseEmitterPin(uint8_t pin)
    {
      pinMode(pin, INPUT);
    }

    virtual void activateEmitterPin(uint8_t pin)
    {
      digitalWrite(pin, HIGH);
    }

    virtual void deactivateEmitterPin(uint8_t pin)
    {
      digitalWrite(pin, LOW);
    }

    // The library uses these functions to help decide if a wait delay can be
    // skipped based on the current state of the emitter pin (e.g. if the
    // emitters are already on, we don't need to wait for them to turn on
    // again). They should return true only if the emitter pin is definitely
    // known to be in the corresponding state. If it isn't possible to determine
    // the state of the emitter pin, both functions should return false.

    virtual bool emitterPinIsActive(uint8_t pin)
    {
      return (digitalRead(pin) == HIGH);
    }

    virtual bool emitterPinIsInactive(uint8_t pin)
    {
