...
void QTRSensors::setEmitterPins(uint8_t oddEmitterPin, uint8_t evenEmitterPin)
{
  ...
  initEmitterPin(_oddEmitterPin);
  initEmitterPin(_evenEmitterPin);
...
void QTRSensors::releaseEmitterPins()
{
  ...
  {
    releaseEmitterPin(_oddEmitterPin);
    _oddEmitterPin = QTRNoEmitterPin;
  }

  if (_evenEmitterPin != QTRNoEmitterPin)
  {
    releaseEmitterPin(_evenEmitterPin);
...
void QTRSensors::emittersOff(QTREmitters emitters, bool wait)
{
      ...
      (_emitterPinCount == 2 && emitters == QTREmitters::Odd))
  {
    ...
    if ((_oddEmitterPin != QTRNoEmitterPin) &&
        !emitterPinIsInactive(_oddEmitterPin))
    {
      deactivateEmitterPin(_oddEmitterPin);
      pinChanged = true;
    }
  }
      ...
      (emitters == QTREmitters::All || emitters == QTREmitters::Even))
  {
    // Check if pin is defined and only turn off if not already off
    if ((_evenEmitterPin != QTRNoEmitterPin) &&
        !emitterPinIsInactive(_evenEmitterPin))
    {
      deactivateEmitterPin(_evenEmitterPin);
      pinChanged = true;
    }
  }
...
void QTRSensors::emittersOn(QTREmitters emitters, bool wait)
{
      ...
      (_emitterPinCount == 2 && emitters == QTREmitters::Odd))
  {
    ...
    // care of this)
    if ((_oddEmitterPin != QTRNoEmitterPin) &&
        ( _dimmable || !emitterPinIsActive(_oddEmitterPin)))
    {
      ...
      (emitters == QTREmitters::All || emitters == QTREmitters::Even))
  {
    ...
    // care of this)
    if ((_evenEmitterPin != QTRNoEmitterPin) &&
        (_dimmable || !emitterPinIsActive(_evenEmitterPin)))
    {
...
{
  if (_dimmable && !emitterPinIsInactive(pin))
  {
    ...
    deactivateEmitterPin(pin);
    delayMicroseconds(1200);
  }

  activateEmitterPin(pin);
